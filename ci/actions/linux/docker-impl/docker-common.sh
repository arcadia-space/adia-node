...
if [[ "$GITHUB_WORKFLOW" != "Develop Branch Dockers Deploy" ]]; then
    docker_image_name="pawdigital/paw${network_tag_suffix}"
fi
...
docker_build()
{
    ...
    if [[ "$GITHUB_WORKFLOW" != "Develop Branch Dockers Deploy" ]]; then
        ghcr_image_name="ghcr.io/${GITHUB_REPOSITORY}/paw${network_tag_suffix}"
...
docker_deploy()
{
    if [ -n "$DOCKER_PASSWORD" ]; then
        echo "$DOCKER_PASSWORD" | docker login -u pawdigital --password-stdin
        if [[ "$GITHUB_WORKFLOW" = "Develop Branch Dockers Deploy" ]]; then
