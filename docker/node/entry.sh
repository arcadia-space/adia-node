...
usage() {
	printf "Usage:\n"
	printf "  $0 arcadia_node [daemon] [cli_options] [-l] [-v size]\n"
	printf "    daemon\n"
	printf "      start as daemon\n\n"
	printf "    cli_options\n"
	printf "      arcadia_node cli options <see arcadia_node --help>\n\n"
	printf "    -l\n"
	...
	printf "      usage\n\n"
	printf "default:\n"
	printf "  $0 arcadia_node daemon -l\n"
}
...
if [ "$1" = 'arcadia_node' ]; then
	command="${command}arcadia_node"
	shift
	for i in $@; do
		case $i in
		"daemon")
