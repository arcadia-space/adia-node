...
void nano::json_handler::process_request (bool unsafe_a)
{
	try
	{
		...
		else
		{
			...
			else if (action == "paw_to_raw")
			{
				paw_to_raw ();
			}
			else if (action == "raw_to_paw")
			{
				raw_to_paw ();
			}
...
void nano::json_handler::paw_to_raw ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
...
void nano::json_handler::raw_to_paw ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
...
void nano::json_handler::version ()
{
	...
	response_l.put ("node_vendor", boost::str (boost::format ("Arcadia %1%") % NANO_VERSION_STRING));
